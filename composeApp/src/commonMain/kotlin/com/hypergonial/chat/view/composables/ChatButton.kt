 ...
 * [FloatingActionButton], and should be used for important, final actions that complete a flow, like "Save", "Join
 * now", or "Confirm".
 *
 * @sample androidx.compose.material3.samples.ButtonSample
 * @sample androidx.compose.material3.samples.ButtonWithIconSample
 *
 * Choose the best button for an action based on the amount of emphasis it needs. The more important an action is, the
 * higher emphasis its button should be.
 ...
 * @param enabled controls the enabled state of this button. When `false`, this component will not respond to user
 *   input, and it will appear visually disabled and disabled to accessibility services.
 * @param shape defines the shape of this button's container, border (when [border] is not null), and shadow (when using
 *   [elevation])
 * @param colors [ButtonColors] that will be used to resolve the colors for this button in different states. See
 *   [ButtonDefaults.buttonColors].
 * @param elevation [ButtonElevation] used to resolve the elevation for this button in different states. This controls
 *   the size of the shadow below the button. See [ButtonElevation.shadowElevation].
